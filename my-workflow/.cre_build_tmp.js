...
var fetchPerson = (nodeRuntime) => {
  ...
  const parsed = json(resp);
  return parsed.result.properties;
};
var onCronTrigger = (runtime2) => {
  runtime2.log("Hello, Star Wars! Workflow triggered.");
  const person = runtime2.runInNodeMode(fetchPerson, consensusIdenticalAggregation())().result();
  runtime2.log(`Successfully fetched person: ${person.name} (${person.height} cm, ${person.mass} kg, born ${person.birth_year})`);
  return {
    person
  };
};
